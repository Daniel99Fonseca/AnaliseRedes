...
O parâmetro de heterogeneidade mede o quão desigual é a distribuição de graus da rede.Para a rede gerada, o valor obtido foi de **`r round(H, 3)`**.

Como este valor é superior a 1, indica que a rede é **bastante heterogénea**, ou seja, existe uma grande variação no número de ligações entre os nodos. A maioria dos nodos apresenta poucos contactos, enquanto um pequeno número de nodos concentra um número muito elevado de ligações.

Isto é característico do modelo de *preferential attachment*, onde nós mais antigos tendem a acumular mais ligações ao longo do tempo.
...
A associação de grau foi calculada, tendo sido obtido o valor de **`r round(assor, 3)`**. Como o valor é **negativo**, conclui-se que a rede é **desassortativa**, o que significa que os nodos com grau mais elevado tendem a ligar-se preferencialmente a nodos com grau mais baixo.

Este comportamento é típico de redes geradas pelo modelo de *preferential attachment*, nas quais existem alguns nós altamente conectados (hubs) que funcionam como pontos centrais de ligação para muitos nós periféricos com poucas ligações.
...
A média dos comprimentos dos caminhos mais curtos obtida foi de **`r round(avg_path, 3)`**, enquanto o diâmetro da rede foi de **`r diam`**.

Estes resultados indicam que, em média, são necessários cerca de **`r round(avg_path, 2)`** passos para ligar dois nodos da rede. O diâmetro confirma que mesmo os nodos mais distantes estão separados por um número reduzido de passos.
...
O coeficiente de clustering da rede foi calculado através da função `transitivity()`, tendo sido obtido o valor de clustering médio de **`r round(clust_media, 3)`**.

Este valor indica a tendência dos nodos da rede formarem triângulos, ou seja, situações em que dois nodos ligados a um terceiro também estão ligados entre si. 

Com base no valor obtido, conclui-se então que existe uma formação de triângulos na rede, embora não de forma muito intensa, o que é consistente com redes do tipo *preferential attachment*, onde a estrutura tende a ser menos agrupada em pequenos clusters fortemente conectados.
...
Devido à remoção dos nodos e consequentemente a perda de **`r rem_nod`** ligações, a nova rede deixou de ser conexa, pois um nodo perdeu todas a ligações ficando isolado da rede.
...
-   **1o Quartil** : `r quantile(graus_q2,0.25)`
-   **3o Quartil** : `r quantile(graus_q2,0.75)`
...
mtext("Distribuição de Grau da Nova Rede", outer = TRUE, cex = 2)
```

A distribuição de grau mostra valores entre **`r min(graus_q2)`** e **`r max(graus_q2)`**, com quartis **Q1=`r quantile(graus_q2, probs = 0.25)`**, **Q2(Mediana)=`r median(graus_q2)`** e **Q3=`r quantile(graus_q2, probs = 0.75)`**, o que indica que a maioria dos nodos possui entre `r round(quantile(graus_q2, probs = 0.25))` e `r round(quantile(graus_q2, probs = 0.75))` ligações.
